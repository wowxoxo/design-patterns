...
private class BridgeRealWorld: XCTestCase {
  ...
  func push(_ container: SharingSupportable) {
    ...
    container.acccept(service: instagram)
    container.update(content: foodModel)

    container.acccept(service: facebook)
    container.update(content: foodModel)
  }

  var foodModel: Content {
    return FoodDomainModel(title: "This food is so various and delicious", images: [UIImage(), UIImage()], calories: 47)
  }
...
class BaseViewController: UIViewController, SharingSupportable {
  ...
  func update(content: Content) {
    // updating UI and showing content
  ...
  func acccept(service: SharingService) {
...
class FaceBookSharingService: SharingService {
  func share(content: Content) {
    ...
    print("Service: \(content) was posted to Facebook")
  }
}
