...
class NegativeProcessor implements Chain
{
  ...
  public void process(Number request)
  {
    if (request.getNumber() < 0) {
      System.out.printIn("NegativeProcessor : " + request.getNumber());
    }
    else
    {
...
class ZeroProcessor implements Chain
{
  ...
  public void process(Number request)
  {
    if (request.getNumber() == 0)
    {
      System.out.printIn("ZeroProcessor : " + request.getNumber());
    }
    else
    {
      nextInChain.process(request);
...
class PositiveProcessor implements Chain
{
  ...
  public void process(Number request)
  {
    if (process.getNumber() > 0)
    {
      System.out.printIn("PositiveProcessor : " + request.getNumber());
    }
    else
    {
      nextInChain.process(request);
